...
- **v0.4.0 ✅** Faster, history-aware replies — in-call "Mex, …" answers now run on a fast model with the no-tools brain stripped of per-call repo/MCP overhead (~12s → ~4s), and when a mex scaffold is present the active loop reads a bounded slice of the event log so it can answer cross-call questions ("what did we decide about X?", "did we already agree on Y?") from the repo's real decision history, not just the current call.
